...
train_split = "train"
model_path = '/pers_files/mask_models_pad_mask19/MSE_net'
val_split = "val"
...
net = IOU_Discriminator_Sig_MSE()
x = Model_Tester_Mask(net, f'{model_path}/model225/best_model.pth',trs_x=tx,device=device)
#x = Model_Tester_Mask(net,)
#i = 2
#x0,x1 = 0.8,0.94
#img, iou = dt_val[i]
#if iou < x0:
...
def get_accs(x0,x1):
    ...
    for i in range( len( dt_val)):
        ...
        iou_pred_tr = x.get_evaluation(img.unsqueeze(0),need_sig=False)
    ...
    print(df.mean())
    print(df.std())
    df.to_csv(f"{model_path}/model225/score_dftest")

get_accs(0.8,0.94)

import pandas as pd
df = pd.read_csv(f"{model_path}/model225/score_dftest.csv")
df['cat_pred']
